...
export default function Socials() {
  return (
    <div className='flex space-x-6 pb-20 md:order-2'>
      {navigation.map(item => (
        <a
          key={item.name}
          href={item.href}
          ...
          className='group relative text-muted-foreground transition-colors delay-150 duration-300 hover:text-foreground'
        >
          <span className='sr-only'>{item.name}</span>
          <item.icon aria-hidden='true' className='h-8 w-8' />
          <span className='pointer-events-none absolute left-1/2 top-full z-10 mt-2 -translate-x-1/2 whitespace-nowrap rounded bg-foreground px-2 py-1 text-xs text-background opacity-0 transition-opacity duration-200 group-hover:opacity-100'>
            {item.name}
          </span>
        </a>
      ))}
